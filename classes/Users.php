...
<strong>Error !</strong> Email already Exists, please try another Email... !</div>';
      ...
      $sql = "INSERT INTO FacultyFacFirstName, FacLastName, FacOfficePhone, FacPassword, FacLocation, FacPhone,FacEmail,DepID,RoldID) VALUES(:facfname, :faclname, :facOphone,:password,:faclocation,:facphone,:email,:depid, :roleid)";
      $stmt = $this->db->pdo->prepare($sql);
      $stmt->bindValue(':facfname', $facfname);
      $stmt->bindValue(':faclname', $faclname);
      $stmt->bindValue(':facOphone', $facOphone);
...
// User login Authorization Method2
  ...
  <strong>Error !</strong> Email did not Found, use Register email or password please !</div>';
        ...
        if ($chkActive == TRUE) {
          $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
    <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
    <strong>Error !</strong> Sorry, Your account is Deactivated, Contact The Admin !</div>';
            return $msg;
        }elseif ($logResult) {
...
// Delete User by Id Method2
    ...
    public function CheckOldPassword($userid, $old_pass){
      ...
      $sql = "SELECT Facpassword FROM Faculty WHERE password = :password AND FacID =:id";
      $stmt = $this->db->pdo->prepare($sql);
